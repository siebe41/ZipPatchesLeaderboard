 ...
 * dictate: where a fresh wave's debt chunks start and which way they drift,
 * and which way a chunk's pieces fly apart when it splits. Everything else
 * -- the ship's own physics -- is a consequence of the player's inputs, so a
 * run replays from its seed and its input trace alone.
 *
