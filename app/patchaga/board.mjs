...
function renderHead(view) {
  ...
  else el.head.appendChild(cell('th', 'Wave', 'num'));
...
function renderRow(row, view, key, pinned) {
  ...
  else tr.appendChild(cell('td', row.wave || '', 'num'));
...
function renderStats(summary) {
  ...
  for (const run of summary.recent || []) {
    ...
    tr.appendChild(cell('td', run.wave, 'num'));
    tr.appendChild(cell('td', run.bugs, 'num'));
