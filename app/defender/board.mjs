...
const VIEWS = [
  {
    ...
    blurb: 'Best single run each player has ever posted.', scoreHead: 'Best run',
  },
  {
  ...
  },
  {
    ...
    blurb: 'Best single run since midnight.', scoreHead: 'Best run',
  },
  {
    id: 'volume', label: 'Most patched',
...
function renderStats(summary) {
  ...
  const items = [
    ...
    ['Furthest level', summary.furthest_level],
    ['Zero-days stopped', summary.intercepts],
    ['Runs', summary.runs],
  ];
  ...
  for (const r of summary.recent || []) {
    const tr = document.createElement('tr');
    tr.appendChild(cell('td', r.score, 'score'));
    tr.appendChild(cell('td', r.level, 'num'));
    tr.appendChild(cell('td', r.intercepts, 'num'));
    tr.appendChild(cell('td', fmtDuration(r.duration_ms), 'num'));
    tr.appendChild(cell('td', new Date(r.created_at).toLocaleString([], {
