...
const API = '/ducker/api/';
const RESTART_LOCKOUT_MS = 800; // stops the death keypress restarting the run
const BEAT_MS = 5000;           // matches BEAT_INTERVAL_MS in ducker.py
 ...
 * seed the client can shop around for: the rules in this folder can be
 * imported and searched offline for a lane pattern that happens to play
 * itself. One request ahead of time keeps the first keypress instant.
 */
...
function startRun() {
  ...
  panelLines([]);
  setStatus('Arrows or WASD to hop. M mutes.', '');
...
async function submitRun() {
  ...
  if (sim.score <= 0) {
    panelLines([{ text: 'NO ENDPOINTS COVERED' }, { text: 'NOTHING TO POST' }]);
  ...
  try {
    ...
    const lines = [{ text: 'POSTED AS ' + (data.player || player), color: '#8ac926' }];
...
function bindInput() {
  window.addEventListener('keydown', (ev) => {
    ...
    ev.preventDefault();
    if (ev.repeat) return; // a hop is one press, however long it is held
    audio.unlock();
...
function frame(nowMs) {
  ...
  if (sim) {
    ...
    if (guard >= CONFIG.maxCatchUpSteps) {
      ...
      // several seconds simulating a crossing nobody was playing.
...
async function boot() {
  ...
  prefetchSession();
  panelLines([
    { text: 'DUCKER', color: '#ffd23f', big: true },
    { text: 'PRESS ANY ARROW TO START', blink: true },
    { text: 'ARROWS OR WASD TO HOP', color: '#9aa4b2' },
  ]);
  setStatus('Arrows or WASD to hop. M mutes.', '');

  if (new URLSearchParams(location.search).has('debug')) {
    window.__ducker = { view, run, CONFIG, STATE, ACTION, queueInput, step };
  }
