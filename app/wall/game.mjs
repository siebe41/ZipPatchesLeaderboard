...
/**
 * Held keys, kept here rather than read from the sim. A stack rather than a
 * pair of booleans so that holding UP, then also pressing DOWN, then
 * releasing DOWN, goes back to UP instead of stopping.
 */
const input = { steer: [], dir: ACTION.NEUTRAL };
...
function startRun() {
  if (!nextSession) {
    setStatus('Getting a fresh match...', '');
    panelLines([{ text: 'GETTING A MATCH...' }]);
  ...
  panelLines([]);
  setStatus('Arrows or W/S to move. M mutes.', '');
  prefetchSession();
...
async function submitRun() {
  ...
  if (sim.score <= 0) {
    panelLines([{ text: 'NOTHING BLOCKED' }, { text: 'NOTHING TO POST' }]);
  ...
  try {
    ...
    const lines = [{ text: 'POSTED AS ' + (data.player || player), color: '#36a2eb' }];
...
function bindInput() {
  window.addEventListener('keydown', (ev) => {
    ...
    if (!KEY_UP.has(ev.code) && !KEY_DOWN.has(ev.code)) return;
    ev.preventDefault();
    audio.unlock();
    const now = performance.now();
    if (tryStart(now)) {
      pressSteer(KEY_UP.has(ev.code) ? ACTION.UP : ACTION.DOWN, now);
      return;
    }
    if (ev.repeat) return;
    pressSteer(KEY_UP.has(ev.code) ? ACTION.UP : ACTION.DOWN, now);
  ...
  window.addEventListener('keyup', (ev) => {
    if (!KEY_UP.has(ev.code) && !KEY_DOWN.has(ev.code)) return;
    releaseSteer(KEY_UP.has(ev.code) ? ACTION.UP : ACTION.DOWN, performance.now());
  });
  ...
  el.canvas.addEventListener('touchmove', (ev) => {
    ...
    const dir = dy < 0 ? ACTION.UP : ACTION.DOWN;
    if (input.dir !== dir) {
      input.steer = [dir];
...
async function boot() {
  ...
  prefetchSession();
  panelLines([
    { text: 'PATCH WALL', color: '#36a2eb', big: true },
    { text: 'PRESS ARROWS TO START', blink: true },
    { text: 'ARROWS OR W/S TO MOVE', color: '#9aa4b2' },
  ]);
  setStatus('Arrows or W/S to move. M mutes.', '');

  if (new URLSearchParams(location.search).has('debug')) {
    window.__wall = { view, run, CONFIG, STATE, ACTION, queueInput, step };
  }
