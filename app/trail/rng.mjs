 ...
 *
 * The simulation uses this for the one thing the tick does not dictate:
 * where the next patch spawns on the grid. Everything else -- the trail's
 * own movement -- is a consequence of the player's inputs, so a run replays
 * from its seed and its input trace alone.
 *
